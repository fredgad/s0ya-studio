...
circleTop.addEventListener('click', (e) => {
    circleAngle += 36;
    bigCircleCont.style.transform = `rotateX(${circleAngle}deg)`;
});
circleBot.addEventListener('click', (e) => {
    circleAngle -= 36;
    bigCircleCont.style.transform = `rotateX(${circleAngle}deg)`;
});
circleTop.addEventListener('mouseenter', (e) => {
    circleAngle += 36;
    bigCircleCont.style.transform = `rotateX(${circleAngle}deg)`;
});
circleBot.addEventListener('mouseenter', (e) => {
    circleAngle -= 36;
    bigCircleCont.style.transform = `rotateX(${circleAngle}deg)`;
});
...
bannerPrev.addEventListener('click', (e) => {
  ...
  banner.className = `banner ban${bannerLet}`;
});
